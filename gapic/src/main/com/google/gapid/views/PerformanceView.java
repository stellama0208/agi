    ...
    private void addColumnForMetric(Service.ProfilingData.GpuCounters.Metric metric) {
      TreeViewerColumn column = addColumn(metric.getName() + "(" + metric.getUnit() + ")", node -> {
        ...
        if (data == null) {
          ...
          if (showEstimate) {
            return perf.getEstimate() < 0 ? "" : String.valueOf(perf.getEstimate());
          } else {
            String minStr = perf.getMin() < 0 ? "?" : String.valueOf(perf.getMin());
            String maxStr = perf.getMax() < 0 ? "?" : String.valueOf(perf.getMax());
